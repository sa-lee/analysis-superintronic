...
## Overlaps with known NMD targets
...
and restrict down to the gene_id and 
...
We then take these results and produce an UpSet plot as we had before:
```{r, echo = TRUE}
si_hits <- totalRNA_sets %>% 
...
nmd_combine <-   nmd_targets_filtered %>% 
  full_join(si_hits) %>%
  mutate_at(vars(-gene_id, -adj_pvalue, -number, -gene_name),
            .funs = ~ if_else(is.na(.), 0L, .))
...
```{r save-plot}
pdf(here::here("img", "nmd-superintronic-olaps.pdf"))
print(nmd_sets)
dev.off()
```
