...
# download gencode files if they don't exist
if (!dir.exists(here::here("data-raw"))) dir.create(here::here("data-raw"))

gtf <- "ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_27/gencode.v27.annotation.gtf.gz"

gtf_dest <- here::here("data-raw", "gencode.v27.annotation.gtf.gz")
if (!file.exists(gtf_dest)) download.file(gtf, gtf_dest)

fa <- "ftp://ftp.ebi.ac.uk/pub/databases/gencode/Gencode_human/release_27/gencode.v27.transcripts.fa.gz"
fa_dest <- here::here("data-raw", "gencode.v27.transcripts.fa.gz")
if (!file.exists(fa_dest)) download.file(fa, fa_dest)
...
At the gene level there are around `r nrow(is_res)` genes declared significant
at 0.05 fdr cut-off between the celllines kits that
...
for (i in seq_len(nrow(top10))) {
  input <- top10[i,]
  cplot <- pretty_cov_plot(cvg, parts, input, design, heights = c(2, 0.25))
