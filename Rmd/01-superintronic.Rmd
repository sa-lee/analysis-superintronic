...
coverage_plot <- function(.y) {
  ...
  cvg_rng <- filter_by_overlaps(cvg_over_features, target) %>% 
  ...
  p <- view_coverage(cvg_rng, 
    ...
    labs(subtitle = paste("Coverage over",
                           target$gene_name, "-", as.character(target)),
         y = "average log coverage"
    ) +
    theme_bw() +
...
for (gene in seq_len(nrow(all_sets))) {
  track_plot <- coverage_plot(all_sets[gene,])
...
for (gene in seq_len(nrow(three_sets))) {
  ...
  track_plot <- coverage_plot(input)
  ggsave(here::here("img", 
